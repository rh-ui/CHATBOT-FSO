...
def analyze_csv_file(file_path):
    """
    Analyze the CSV file to identify issues
    """
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            for line_num, line in enumerate(file, 1):
                ...
                # If more than 1 comma and not properly quoted
                if comma_count > 1:
...
def fix_csv_file(input_file, output_file):
    """
    Fix the CSV file by properly handling commas in questions
    """
    ...
    try:
            ...
            for line_num, line in enumerate(file, 1):
                ...
                try:
                    ...
                    if ',' in line:
                        ...
                        print(f"⚠️ No comma found in line {line_num}: {line[:50]}...")
                        errors += 1
        ...
        print(f"📊 Processed {len(fixed_lines)} valid lines")
        ...
        try:
            ...
            print(f"✅ Fixed file loads successfully with {len(test_df)} rows and {len(test_df.columns)} columns")
            
            if len(test_df.columns) == 2:
                test_df.columns = ['question', 'intent']
                print(f"📊 Unique intents: {test_df['intent'].nunique()}")
                print(f"📊 Sample data:")
                for i in range(min(3, len(test_df))):
                    print(f"  Q: {test_df.iloc[i]['question'][:60]}...")
                    print(f"  I: {test_df.iloc[i]['intent']}")
...
def main():
    """
    Main function to analyze and fix CSV file
    """
    print("🛠️ CSV File Fixer and Validator")
    print("=" * 50)
