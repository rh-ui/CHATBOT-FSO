...
class StreamGenerator:
    ...
    def stream_search(self):  # using this
        ...
        try:
            ...
            list_results = llm_service.simplify_question(
                self.query.question, self.query.lang
            )
            ...
            for i, res in enumerate(list(list_results)):
                ...
                logger.info(
                    f"Processing question {i+1} ({question_type}): {question_text}"
                )
                ...
                if question_type == "static":
                    intent, probabilities = classifier.predict_intent(
                        question_text.lower(), return_probabilities=True
                    )

                    logger.info(f"Question {i+1} - Main predicted intent: {intent}")
                    logger.info(
                        f"Question {i+1} - Top 3 intents: {list(probabilities.items())[:3]}"
                    )
                    ...
                    for pred_intent, prob in list(probabilities.items())[:3]:
                        if prob > 0.2:
                            docs = index_faq_data(
                                DATASET, pred_intent, self.query.lang, prob
                            )
                    ...
                    logger.info(
                        f"Question {i+1} - Database documents found: {len(question_documents)}"
                    )
                    ...
                    if not question_documents:
                        ...
                        logger.info(
                            f"Question {i+1} - No database documents found, will search internet"
                        )
                    ...
                    # Condition 2: Check relevance of database results using LLM
                        ...
                        try:
                            ...
                            if not is_relevant:
                                need_internet_search = True
                                logger.info(
                                    f"Question {i+1} - Database results not relevant, will search internet"
                                )
                            else:
                                logger.info(
                                    f"Question {i+1} - Database results are relevant, using database only"
                                )
                                need_internet_search = False

                        except Exception as e:
                            logger.error(
                                f"Question {i+1} - Relevance check failed: {str(e)}"
                            )
                            # Fallback: assume results are relevant if check fails
                            logger.info(
                                f"Question {i+1} - Using database results due to relevance check failure"
                            )
                    ...
                    if need_internet_search:
                        ...
                        try:
                            internet_results = get_internet_results_for_question(
                                question_text, self.query.lang
                            )
                            if internet_results:  # Check if results exist
                                ...
                                logger.info(
                                    f"Question {i+1} - Internet documents found: {len(internet_results)}"
                                )
                            else:
                                logger.warning(
                                    f"Question {i+1} - No internet results returned"
                                )
                        except Exception as e:
                            logger.error(
                                f"Question {i+1} - Internet search failed: {str(e)}"
                            )

                elif question_type == "dynamic":
                    # Condition 3: Always search internet for dynamic questions
                    logger.info(
                        f"Question {i+1} - Dynamic question, using internet search"
                    )
                    ...
                    try:
                        ...
                        if internet_results:
                            ...
                            logger.warning(
                                f"Question {i+1} - No internet results for dynamic question"
                            )
                    except Exception as e:
                        logger.error(
                            f"Question {i+1} - Internet search failed for dynamic question: {str(e)}"
                        )
                ...
                question_answer_pairs.append(
                    {
                        ...
                        "source": (
                            "database" if question_type == "static" else "internet"
                        ),
                    }
                )
            ...
            if not all_documents:
                logger.info(
                    "No documents found from any source, returning no results message"
                )
                final_result = {
            ...
            if self.query.use_llm:
                relevant_pairs = [
                    pair for pair in question_answer_pairs if pair["documents"]
                ]

                if not relevant_pairs:
                    logger.info(
                        "No questions with documents found, returning no results"
                    )
                    final_result = {
                        "detected_lang": self.query.lang,
                        "structured_response": get_no_results_message(self.query.lang),
                ...
                try:
                    llm_response = (
                        llm_service.generate_comprehensive_response_optimized(
                            original_question=self.query.question,
                ...
                if self.query.context:
                    try:
                        llm_response["response"] = (
                            llm_service.enhance_response_with_context(
                                llm_response["response"],
                                self.query.context,
                                self.query.lang,
                            )
                ...
                sources_used = set(
                    [pair["source"] for pair in relevant_pairs if pair["source"]]
                )
                search_source = (
                    "mixed"
                    if len(sources_used) > 1
                    else list(sources_used)[0] if sources_used else "none"
                )
                final_result = {
                    "detected_lang": self.query.lang,
                    ...
                    "documents_found": len(
                        [doc for pair in relevant_pairs for doc in pair["documents"]]
                    ),
                    "llm_calls_made": 2 + (1 if self.query.context else 0),
                    "question_coverage": llm_response.get("question_coverage", {}),
                    "sources_breakdown": {
                        "database": len(
                            [p for p in relevant_pairs if p["source"] == "database"]
                        ),
                        "internet": len(
                            [
                                p
                                for p in relevant_pairs
                                if p["source"] in ["internet", "internet_fallback"]
                            ]
                        ),
                        "mixed": len(
                            [p for p in relevant_pairs if p["source"] == "mixed"]
                        ),
                    },
